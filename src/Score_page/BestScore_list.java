...
public class BestScore_list {  
  	...
  	public static void main (String args []){	
    	...
    	String s;
    	try {
    		con = DriverManager.getConnection("jdbc:mysql://localhost/g14?"
					+ "user=root&password=0000t&serverTimezone=UTC&useSSL=false");
    		st = con.createStatement();
    		s = "select * from player ORDER BY score ASC";//?‰é€?score ?±å??’åˆ°å¤?
            ...
            Exit.addActionListener(new ActionListener(){ 
        		public void actionPerformed(ActionEvent e){ 
        			windowClosing(null);//è·³å‡º?¢é?è¦–ç?
        			} 
        		}); 
    		...
    		JTable table = new JTable(data,column) { // ?®å??­jtableï¿½ï¿½?’î?ï¿½ï¿½?¶î¹µ?ç²¹ï¿½î²ï¿½î?ï¿½ï¿½
    		...
    		//table è¨­å?
    		table.setRowHeight(30);// è¡Œè?
    		table.setFont(new Font("æ¨™æ¥·é«?, Font.PLAIN, 18));
    		table.setForeground(Color.BLACK);
    		DefaultTableCellRenderer tcr = new DefaultTableCellRenderer();//?®å??­tableï¿½ï¿½?¯æ??…ï†®??
            ...
            head.setFont(new Font("æ¨™æ¥·é«?, Font.PLAIN, 18));
    ...
    public static void windowClosing(WindowEvent e){
        int result=JOptionPane.showConfirmDialog(frame, "?¯å¦ç¢ºå?è¦é›¢??", "ç¢ºå???", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
            
        if (result==JOptionPane.YES_OPTION){
        	frame.dispose();
            System.exit(0);  
        }else{
        	frame.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);//???™è????™è????™è????
        }            
    }  
